...
function carouse(ele, clsn) {
	...
	// var prevBtn = box.getElementsByClassName('pre')[0];
	// console.log(prevBtn);
...
/*
	 ...
	 功能 —— 1、自动轮播
			  ...
			  4、点击焦点切换到相应图片
 */
function carousel1(ele, clsn) {
	// 获取节点
	var box = getid(ele); //最外层的节点	
	var ul = box.getElementsByClassName('ul2')[0];
	var alis = ul.getElementsByTagName('li');
	var light = box.getElementsByClassName('light2')[0];
	var aspans = light.getElementsByTagName('span');
	var prevBtn = box.getElementsByClassName('pre2')[0];
	var nextBtn = box.getElementsByClassName('next2')[0];
	// 声明变量
	var now = 0; //当前可视区图片下标
	var nowIndex = 2; //层级
	// 1、自动跳转
	...
	function tab() {
		alis[now].style.zIndex = nowIndex++;
		alis[now].style.opacity = 0; //渐现
	...
	for (var i = 0; i < aspans.length; i++) {
		...
		aspans[i].onmousemove = function () {
			console.log(this.index);
			alis[this.index].style.zIndex = nowIndex++;
			for (var i = 0; i < aspans.length; i++) {
				aspans[i].className = '';
			}
			aspans[this.index].className = clsn;
		};
	}
}
...
/*
	 点击按钮回到顶部（缓慢）
	 功能 —— 1、按钮先消失，到达一定高度，按钮出现
			  2、点击按钮缓慢的回到顶部
 */
function comeback() {
	//获取节点
	var combackBtn = document.getElementById("comeback-top");
	// 滚轮滚动时间：获取滚动的Y距离
	window.onscroll = function () {
		var scrollTop = window.scrollY;
		//判断：滚动滑轮到175px出现按钮，之前按钮隐藏
		if (scrollTop >= 160) {
			css(combackBtn, 'display', 'block');
		} else {
			css(combackBtn, 'display', 'none');
		};
		//点击事件：点击按钮，回到顶部
		combackBtn.onclick = function () {
...
/*
	 ...
	 功能 —— 人工服务先隐藏，鼠标滑过按钮出现
 */
function service(ele, num) {
	//6、滑过按钮打开隐藏的人工服务
	var service = document.getElementById(ele);
	console.log(service);
	//鼠标经过的时候，box显示，鼠标离开就隐藏
	service.onmouseover = function () {
		startMove(service, { 'right': num });
	};
	service.onmouseout = function () {
		startMove(service, { 'right': -600 });
	};
}

/*
	点击按钮回到顶部（缓慢） &&  滚动滑轮，菜单根据可视窗口的scrollY进行相对应高度的变化
		 功能 —— 1、按钮先消失，到达一定高度，按钮出现
			  2、点击按钮缓慢的回到顶部
			  3、滚轮到达一定高度后，菜单相应改变高度
 */
function meumscroll() {
	window.onscroll = function () {
		// 点击按钮回到顶部（缓慢）
		...
		// 滚动滑轮，菜单根据可视窗口的scrollY进行相对应高度的变化
		//获取节点
		var num2 = document.getElementsByClassName("twomeum");
		//声明变量获取窗口垂直高度；
		var scroll = window.scrollY;
		for (var i = 0; i < num2.length; i++) {
			if (scroll >= 200) {
				// 滚动至200以上，菜单随滚轮的滚轮相应滚动
				var scrollres = scroll - 202;
				startMove(num2[i], { "top": scrollres });
			} else {
				// 小于200，取消固定定位，高度为0
				startMove(num2[i], { "top": 0 });
			}
		}
	};
}